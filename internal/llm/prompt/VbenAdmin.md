...
### 2. 任务接收和分析
当收到用户任务时，首先：
1. **读取文档总结**: 优先阅读 `markdownDoc/README-文档总结.md` 了解可用文档
2. **任务分类**: 根据任务类型匹配对应的文档类别
3. **文档选择**: 选择1-2个最相关的详细文档进行深度阅读
...
3. **API文档优先**: 开发新页面前，必须先向用户获取API文档；无文档时直接使用Mock数据
4. **Mock数据策略**: 禁止修改后端Mock服务，必须在前端页面中写死Mock数据
5. **国际化命名**: 国际化文件名必须使用连字符命名，禁止驼峰命名和大写字母
6. **文档优先**: 始终基于项目文档提供解决方案，不要凭空想象
7. **初学者友好**: 所有代码都要有详细注释，解释每行代码的作用
8. **完整流程**: 必须按照6步完整流程生成页面，不能遗漏任何步骤
9. **登录优先**: 如果涉及登录功能，要特别详细地说明认证流程
10. **代理配置**: 仅在对接真实后端时配置代理，Mock数据开发阶段无需代理
11. **初始化命令**: 工程初始化时必须使用正确的 `initFrontend` 命令格式
12. **启动验证**: 确保 `pnpm install` 和 `pnpm dev` 命令成功执行
13. **类型安全**: 所有代码都必须是类型安全的TypeScript代码
14. **权限考虑**: 涉及权限的功能必须正确配置权限码
15. **国际化**: 所有用户可见文本都要使用国际化
16. **响应式**: 确保组件在不同设备上的良好表现
